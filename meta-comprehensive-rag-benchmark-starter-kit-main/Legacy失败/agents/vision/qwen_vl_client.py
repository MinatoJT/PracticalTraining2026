...
from .visual_anchor import extract_json_object, normalize_rerank, validate_anchor
from .visual_query import build_visual_candidate_view, parse_visual_query_target


DEFAULT_ANCHOR_MODEL = "qwen3.5-omni-plus"
DEFAULT_RERANK_MODEL = "qwen3.5-omni-plus"
...
class QwenVLClient:
    ...
    def __init__(self, model: str, base_url: str, api_key: str, provider: str = "dashscope", rerank_model: Optional[str] = None, fallback_model: Optional[str] = None):
        ...
        # 重排需要返回多候选评分，使用独立上限，避免完整 JSON 被 1024 token 截断。
        self.rerank_max_tokens = int(os.getenv("QWEN_VL_RERANK_MAX_TOKENS", "4096"))
    ...
    def rerank_candidates(
        ...
        if error:
            return {**result, "ok": False, "error": "invalid_json", "raw": result.get("content", "")[:800]}
        rerank, error, diagnostics = normalize_rerank(parsed, len(candidates))
        if error:
            mapped = "selected_index_out_of_range" if "index" in error else "schema_validation_failed"
            return {**result, "ok": False, "error": mapped, "schema_error": error, "raw": result.get("content", "")[:800], "rerank_diagnostics": diagnostics}
        selected_index = int(rerank.get("selected_index", 0) or 0)
        if not rerank.get("no_valid_candidate"):
            expected = str(candidates[selected_index - 1].get("entity_name", "")).strip()
            returned = str(rerank.get("selected_entity", "")).strip()
            if returned.lower() != expected.lower():
                diagnostics.setdefault("warnings", []).append("selected_entity_mismatch_use_index")
                diagnostics["parse_status"] = "recovered"
            rerank["selected_entity"] = expected
        return {**result, "rerank": rerank, "raw": result.get("content", "")[:800], "rerank_diagnostics": diagnostics}
    ...
    def _vision_json(self, image: Image.Image, prompt: str, purpose: str, model: str) -> Dict[str, Any]:
        ...
        max_tokens = self.rerank_max_tokens if purpose == "rerank" else self.max_tokens
        return self._chat_json(messages, purpose, max_tokens, model=model, allow_model_fallback=True)
    ...
    def _rerank_prompt(query: str, anchor: Dict[str, Any], candidates: List[Dict[str, Any]], history: str) -> str:
        compact = [{"index": index, **build_visual_candidate_view(item)} for index, item in enumerate(candidates, 1)]
        target = parse_visual_query_target(query, anchor)
        visual_target = {key: value for key, value in target.items() if key not in {"requested_property", "nonvisual_constraints"}}
        return (
            "You are a visual entity verifier, not a question-answering model. "
            "Your only task is to identify which candidate best matches the visible object referred to by the user. "
            "Do not answer the user's factual question. Do not use world knowledge about dates, events, specifications, origins, capacities, history, safety, or other non-visible facts to select a candidate. "
            "A candidate must not receive a higher score merely because it makes the user's question true or easy to answer. "
            "Use only visible appearance, object category, logos/OCR, shape, color, materials, structure, spatial reference, depiction level, scene consistency, and the visual anchor. "
            "Image similarity is a weak reference. Candidate indices are 1-based. Return JSON only.\n"
            f"Full query (reference resolution only): {query}\n"
            f"Visual target: {json.dumps(visual_target, ensure_ascii=False)}\n"
            f"Visual anchor: {json.dumps(anchor, ensure_ascii=False)}\n"
            f"Candidates: {json.dumps(compact, ensure_ascii=False)}\n"
            "JSON fields: selected_index, selected_entity, confidence, "
            "candidate_scores[{index,appearance_match,target_reference_match,category_match,ocr_match,depiction_level_match,scene_consistency,visual_final_score,reason}], "
            "rejected_indices, no_valid_candidate. selected_entity must correspond to selected_index."
        )

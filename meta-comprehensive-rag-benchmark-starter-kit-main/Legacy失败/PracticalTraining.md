...
## 受控修复：纯视觉重排、分层回退与证据重试（2026-07-13）

本轮从已验收 HEAD 出发，以固定 10 个回归样本做 A/B/C/D 消融；不修改官方 evaluator 协议，不使用 Ground Truth 控制运行逻辑，不为失败样本硬编码答案，Task1 不增加 Web。

- `agents/vision/visual_query.py::parse_visual_query_target(...)`：把问题拆成视觉指代、目标大类、待询问知识属性、视觉约束和非视觉约束；非视觉约束不参与视觉候选加分。
- `agents/vision/visual_query.py::build_visual_candidate_view(...)`：为 Qwen 生成视觉字段白名单视图，过滤历史、年份、容量、油耗、起源、价格、营养和安全性等知识字段。
- `agents/vision/qwen_vl_client.py::rerank_candidates(...)`：使用 `qwen3.5-omni-plus` 执行纯视觉实体重排，禁止依据日期、事件、参数等非视觉知识选择实体。
- `QWEN_VL_RERANK_MAX_TOKENS=4096`：重排 JSON 使用独立输出预算；anchor 仍保持 1024，避免多候选评分因 `content_truncated` 全部退回 fallback。
- `agents/vision/visual_anchor.py::normalize_rerank(...)`：规范化 Markdown fence、字符串数字、0/1 基索引和越界置信度；缺少可选评分不再使整次重排失败，但不会生成缺失索引或接受越界索引。
- `agents/vision/visual_candidate_pipeline.py::_hierarchical_fallback(...)`：重排失败时依次尝试高置信 anchor 的实体精确匹配、别名匹配、token 匹配、类别过滤，最后才使用 legacy image top1。
- `agents/Task1KGAgent.py::_maybe_evidence_retry(...)`：初次回答为空或 IDK、实体置信度足够且存在相关证据时，仅重试一次；Task1 只用 KG，Task2/3 可附加已检索 Web 证据。
- `agents/claim_verifier.py`：定义 identity、knowledge、coverage 三类 claim 的验证契约。目前仅提供接口和单测，reliability/verifier 默认关闭，未在 D 优于 C 前接入运行链路。
- `fixed_sample_selection.py::select_rows_by_ids(...)`：按 `sample_ids.txt` 顺序选择固定样本，仅用于诊断入口，不改变 evaluator 的数据与评分协议。
- `UI/run_eval.py --sample-ids-file/--output-dir`：固定样本过滤与隔离输出；配置快照只记录 Key 是否存在，不保存 Key、Base64 或用户敏感信息。
- `UI/run_controlled_repair.ps1`：隐藏输入 API Key，统一模型、revision 和缓存策略，依次执行 A 当前 HEAD、B anchor only、C pure visual rerank、D evidence retry；不会 commit 或 push。

验收后的默认开关为 `VISION_ENABLED=1`、`VISION_RERANK_ENABLED=1`、`VISION_RERANK_MODE=pure_visual`、`ANSWER_RELIABILITY_ENABLED=0`、`VISUAL_VERIFIER_ENABLED=0`、`EVIDENCE_RETRY_ENABLED=0`。证据重试保留用于消融，但因 D 阶段幻觉率高于 C，暂不默认启用。

```powershell
# 确定性测试
C:\anaconda\python.exe -B -m unittest -q testControlledRepair testQwenVision testTask23Diagnostics testTask1

# 真实 A/B/C/D 消融；Key 在终端中隐藏输入
powershell -ExecutionPolicy Bypass -File .\UI\run_controlled_repair.ps1
```

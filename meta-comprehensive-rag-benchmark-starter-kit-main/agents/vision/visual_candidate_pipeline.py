...
class VisualCandidatePipeline:
    ...
    RERANK_PROMPT_VERSION = "dashscope-rerank-v1"

    def __init__(self):
        self.enabled = os.getenv("VISION_ENABLED", "0") == "1"
    ...
    def prepare(self, query: str, image: Image.Image, history: List[Dict[str, Any]], legacy_candidates: List[Dict[str, Any]], cached_anchor: Optional[Dict[str, Any]] = None, refresh_anchor: bool = True, trace: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if refresh_anchor or not anchor:
            ...
            cache_key = f"{image_hash}:{context_fingerprint}:{self.ANCHOR_PROMPT_VERSION}"
            with self._lock:
        ...
        if not self.rerank_enabled:
            ...
            result = {"anchor": anchor, "candidates": ranked, "selected_entity": selected, "rerank": {}, "fallback_used": False, "fallback_reason": "", "mode": "anchor_only", "request_metadata": request_metadata}
            self._log(trace, query, image_hash, anchor, merged, result, analyze_latency, 0.0)
            return result

        candidate_fingerprint = hashlib.sha256(
            json.dumps(
                [{"entity": item.get("entity_name"), "attrs": item.get("attributes", {})} for item in limited],
                sort_keys=True,
                ensure_ascii=False,
            ).encode("utf-8")
        ).hexdigest()[:20]
        rerank_key = f"{image_hash}:{hashlib.sha256(query.strip().lower().encode('utf-8')).hexdigest()[:16]}:{candidate_fingerprint}:{self.RERANK_PROMPT_VERSION}"
        with self._lock:
        ...
        if not rerank_result.get("ok"):
            result = self._fallback(legacy_candidates, rerank_result.get("error", "rerank_failed"), anchor)
            result["request_metadata"] = request_metadata
        ...
        if rerank.get("no_valid_candidate") or float(rerank.get("confidence", 0.0) or 0.0) < self.min_confidence:
            ...
            result = self._fallback(legacy_candidates, reason, anchor)
            result["request_metadata"] = request_metadata
            self._log(trace, query, image_hash, anchor, merged, result, analyze_latency, rerank_latency)
            return result
        ...
        for index, candidate in enumerate(limited, 1):
            ...
            item.update({"qwen_visual_score": score.get("visual_match", 0.0), "qwen_question_score": score.get("question_match", 0.0), "qwen_ocr_score": score.get("ocr_match", 0.0), "qwen_reason": score.get("reason", ""), "qwen_final_score": score.get("final_score", 0.0), "rule_score": float(score.get("final_score", 0.0))})
            ranked.append(item)
        ranked.sort(key=lambda item: (item.get("qwen_final_score", 0.0), item.get("score", 0.0)), reverse=True)
        ...
        result = {"anchor": anchor, "candidates": ranked, "selected_entity": selected, "rerank": rerank, "fallback_used": False, "fallback_reason": "", "mode": "anchor_rerank", "request_metadata": request_metadata}
        self._log(trace, query, image_hash, anchor, merged, result, analyze_latency, rerank_latency)
        return result
    ...
    @staticmethod
    def _normalize_name(value): return " ".join(str(value or "").lower().split())
    @staticmethod
    ...
    def _log(self, trace, query, image_hash, anchor, merged, result, analyze_latency, rerank_latency):
        ...
        payload = {"event": "vision_pipeline", "conversation": (trace or {}).get("session_id"), "turn": (trace or {}).get("turn_idx"), "query": query, "image_hash": image_hash, "anchor_model": getattr(self.client, "anchor_model", ""), "rerank_model": getattr(self.client, "rerank_model", ""), "mode": result.get("mode", "legacy"), "requests": result.get("request_metadata", []), "anchor_success": bool(anchor), "anchor": anchor, "anchor_confidence": anchor.get("confidence", 0.0) if anchor else 0.0, "visible_text": anchor.get("visible_text", []) if anchor else [], "retrieval_queries": anchor.get("retrieval_queries", []) if anchor else [], "candidate_sources": [{"entity": x.get("entity_name"), "sources": x.get("sources", [])} for x in merged[:15]], "selected_entity": (result.get("selected_entity") or {}).get("entity_name"), "selected_confidence": (result.get("rerank") or {}).get("confidence", 0.0), "fallback_used": result.get("fallback_used", True), "fallback_reason": result.get("fallback_reason", ""), "cache_hits_total": self._cache_hits, "analyze_latency": round(analyze_latency, 4), "rerank_latency": round(rerank_latency, 4), "usage": self.client.stats() if self.client and hasattr(self.client, "stats") else {}, "final_candidates": [x.get("entity_name") for x in result.get("candidates", [])[:15]]}
        try:
